...
const Abonamente = () => {
  ...
  useEffect(() => {
    const fetchAbonamentActiv = async () => {
      try {
        ...
        console.error('Eroare Axios:', error.response ? error.response.data : error.message);
        setMesajEroare(error.response ? error.response.data.message : 'Nu s-a putut obține abonamentul activ.');
        setMesajSucces('');
      }
    };
  ...
  const handleAbonare = async () => {
    ...
    try {
      ...
      const response = await axios.post('/abonamente/subscribe', {
  ...
  const handleUpgradeAbonament = async (tipNou) => {
    ...
    try {
      ...
      const response = await axios.put('/abonamente/upgrade', {
        tipNou,
      }, {
        headers: { 'Authorization': `Bearer ${token}` },
      });
      ...
      setMesajEroare('A apărut o eroare la actualizarea abonamentului.');
      setMesajSucces('');
    }
  };
  ...
  const handleDowngradeAbonament = async (tipNou) => {
    ...
    try {
      ...
      const response = await axios.put('/abonamente/switch', {
        tipNou,
      }, {
        headers: { 'Authorization': `Bearer ${token}` },
      });
      ...
      setAbonamentActiv(response.data.abonament);
      setMesajSucces('Abonamentul a fost redus cu succes!');
      setMesajEroare('');
    } catch (error) {
      console.error('Eroare la downgrade:', error.response?.data || error.message);
      setMesajEroare('A apărut o eroare la reducerea abonamentului.');
      setMesajSucces('');
    }
  };
    ...
    <div className="abonamente">
      ...
      {abonamentActiv ? (
        ...
        <div className="abonament-activ">
          ...
          {/* Logica corectă pentru afișarea butoanelor de upgrade și downgrade */}
          {abonamentActiv.tip === 'Standard' && (
            <>
              <button onClick={() => handleUpgradeAbonament('Premium')}>Upgrade la Premium</button>
              <button onClick={() => handleUpgradeAbonament('Anual')}>Upgrade la Anual</button>
            </>
          )}
          {abonamentActiv.tip === 'Premium' && (
            <>
              <button onClick={() => handleUpgradeAbonament('Anual')}>Upgrade la Anual</button>
              <button onClick={() => handleDowngradeAbonament('Standard')}>Downgrade la Standard</button>
            </>
          )}
          {/* ..*/}
          {abonamentActiv.tip === 'Anual' && (
            <>
              <button onClick={() => handleDowngradeAbonament('Premium')}>Downgrade la Premium</button>
              <button onClick={() => handleDowngradeAbonament('Standard')}>Downgrade la Standard</button>
            </>
          )}
        </div>
        ...
        <div className="abonamente-lista">
          {abonamente.map((abonament) => (
            <div
              ...
              style={{
                border: abonamentSelectat?.id === abonament.id ? '2px solid blue' : '1px solid gray',
                padding: '10px',
                margin: '10px',
                cursor: 'pointer',
              }}
